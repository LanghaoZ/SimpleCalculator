...
let nextNumber = (expr, end) => {
    ...
    let num = start < expr.length ? parseFloat(expr.substring(start, end)) : 0;
    return {
        value: num,
        index: end
    };    
...
let convertToPostfix = (expr) => {
    ...
    while (i < n) {
        ...
        if (isValidOperator(expr[i])) {
            if (expr[i] == '-' && (i == 0 || isValidOperator(expr[i - 1]))) {
                let obj = nextNumber(expr, i + 1);
                ...
                i = obj.index;
                continue;
            } else if (operatorStack.length == 0 || computeOrder(expr[i]) > computeOrder(operatorStack[operatorStack.length - 1])) {
